...
void BGDiff::BackgroundDiff_lmn(Mat src, Mat &imgForeground, Mat& imgBackground, int threshold_method)
{
    // 源图像的灰度图像
    ...
    absdiff(src, imgBackground, imgForeground);

    // 复制前景图像
    // Copy Foreground Image
    imgForeground.copyTo(imgForeground_temp);
 ...
 *      - THRESH_OTSU:       使用OpenCV自带OTUS方法
 *      - THRESH_BINARY:   使用该类中的OTSU方法
 ...
 *      - THRESH_OTSU:       Using OpenCV's OTSU method
 *      - THRESH_BINARY:   Using this class's OTSU method
